...
public class ModelChecker {
    public static void main(String[] args)  throws Exception{
        checkInteractive(new File("Boards/level1.smv"), 100);
...
public static void checkInteractive(File smvFile, int bound) throws IOException, InterruptedException, Exception{                   
        String[] commands = new String[]{ "nuXmv", "-bmc", "-bmc_length", Integer.valueOf(bound).toString(),smvFile.getPath()};
        ProcessBuilder pb = new ProcessBuilder(commands);    
        pb.inheritIO();
        
        pb.redirectOutput(new File("a.out"));
